  ...
  const loadOrders = useCallback(
    async (status: 'out_for_delivery' | 'declined', page: number) => {
      ...
      try {
        ...
        showToast(errorMsg, 'error'); // Use directly
    ...
    [], // Empty deps - state setters and showToast are stable
  );
            ...
            <>
              <div className='space-y-4'>
                {data.orders.map((order) => (
                  <div
                    key={order.id}
                    className='rounded-lg border border-gray-700 bg-gray-900/50 p-4 transition-all hover:border-purple-500/50 hover:shadow-lg hover:shadow-purple-500/10'
                  >
